...
# Priors and constraints
   ...
   # Transitions: multinomial logit
      ...
      for (i in 1:3){
         ...
         lpsiD[i] ~ dnorm(0, 0.001)
         }
      # Constrain the transitions such that their sum is < 1
      for (i in 1:3){
         psiA[i] <- exp(lpsiA[i]) / (1 + exp(lpsiA[1]) + exp(lpsiA[2]) + exp(lpsiA[3]))
         psiB[i] <- exp(lpsiB[i]) / (1 + exp(lpsiB[1]) + exp(lpsiB[2]) + exp(lpsiB[3]))
         psiC[i] <- exp(lpsiC[i]) / (1 + exp(lpsiC[1]) + exp(lpsiC[2]) + exp(lpsiC[3]))
         psiD[i] <- exp(lpsiD[i]) / (1 + exp(lpsiD[1]) + exp(lpsiD[2]) + exp(lpsiD[3]))
         }
      # Calculate the last transition probability
      psiA[4] <- 1-psiA[1]-psiA[2]-psiA[3]
      psiB[4] <- 1-psiB[1]-psiB[2]-psiB[3]
      psiC[4] <- 1-psiC[1]-psiC[2]-psiC[3]
      psiD[4] <- 1-psiD[1]-psiD[2]-psiD[3]
