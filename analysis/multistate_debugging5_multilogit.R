...
model {
  ...
  for (t in 1:(n.occasions-1)){
    psiDB[1,t] <- (exp(lpsiDB[1]) / (1 + exp(lpsiDB[1]) + exp(lpsiDB[2]) + exp(lpsiDB[3]) + exp(lpsiDB[4]) + exp(lpsiDB[5]) + exp(lpsiDB[6]))) * season[t]
    psiDB[2,t] <- (exp(lpsiDB[2]) / (1 + exp(lpsiDB[1]) + exp(lpsiDB[2]) + exp(lpsiDB[3]) + exp(lpsiDB[4]) + exp(lpsiDB[5]) + exp(lpsiDB[6]))) * season[t]
    psiDB[3,t] <- (exp(lpsiDB[3]) / (1 + exp(lpsiDB[1]) + exp(lpsiDB[2]) + exp(lpsiDB[3]) + exp(lpsiDB[4]) + exp(lpsiDB[5]) + exp(lpsiDB[6]))) * season[t]
    psiDB[4,t] <- (exp(lpsiDB[4]) / (1 + exp(lpsiDB[1]) + exp(lpsiDB[2]) + exp(lpsiDB[3]) + exp(lpsiDB[4]) + exp(lpsiDB[5]) + exp(lpsiDB[6]))) * season[t]
    psiDB[5,t] <- (exp(lpsiDB[5]) / (1 + exp(lpsiDB[1]) + exp(lpsiDB[2]) + exp(lpsiDB[3]) + exp(lpsiDB[4]) + exp(lpsiDB[5]) + exp(lpsiDB[6]))) * season[t]
    psiDB[6,t] <- (exp(lpsiDB[6]) / (1 + exp(lpsiDB[1]) + exp(lpsiDB[2]) + exp(lpsiDB[3]) + exp(lpsiDB[4]) + exp(lpsiDB[5]) + exp(lpsiDB[6]))) * season[t]
    psiDB[7,t] <- 1 - psiDB[1,t] - psiDB[2,t] - psiDB[3,t] - psiDB[4,t] - psiDB[5,t] - psiDB[6,t] 
  }
  ...
  for (t in 1:(n.occasions-1)){
    psiJB[1,t] <- (exp(lpsiJB[1]) / (1 + exp(lpsiJB[1]) + exp(lpsiJB[2]) + exp(lpsiJB[3]) + exp(lpsiJB[4]) + exp(lpsiJB[5]) + exp(lpsiJB[6]))) * season[t]
    psiJB[2,t] <- (exp(lpsiJB[2]) / (1 + exp(lpsiJB[1]) + exp(lpsiJB[2]) + exp(lpsiJB[3]) + exp(lpsiJB[4]) + exp(lpsiJB[5]) + exp(lpsiJB[6]))) * season[t]
    psiJB[3,t] <- (exp(lpsiJB[3]) / (1 + exp(lpsiJB[1]) + exp(lpsiJB[2]) + exp(lpsiJB[3]) + exp(lpsiJB[4]) + exp(lpsiJB[5]) + exp(lpsiJB[6]))) * season[t]
    psiJB[4,t] <- (exp(lpsiJB[4]) / (1 + exp(lpsiJB[1]) + exp(lpsiJB[2]) + exp(lpsiJB[3]) + exp(lpsiJB[4]) + exp(lpsiJB[5]) + exp(lpsiJB[6]))) * season[t]
    psiJB[5,t] <- (exp(lpsiJB[5]) / (1 + exp(lpsiJB[1]) + exp(lpsiJB[2]) + exp(lpsiJB[3]) + exp(lpsiJB[4]) + exp(lpsiJB[5]) + exp(lpsiJB[6]))) * season[t]
    psiJB[6,t] <- (exp(lpsiJB[6]) / (1 + exp(lpsiJB[1]) + exp(lpsiJB[2]) + exp(lpsiJB[3]) + exp(lpsiJB[4]) + exp(lpsiJB[5]) + exp(lpsiJB[6]))) * season[t]
    psiJB[7,t] <- 1 - psiJB[1,t] - psiJB[2,t] - psiJB[3,t] - psiJB[4,t] - psiJB[5,t] - psiJB[6,t] 
  }
  ...
  for (t in 1:(n.occasions-1)){
    psiMI[1,t] <- (exp(lpsiMI[1]) / (1 + exp(lpsiMI[1]) + exp(lpsiMI[2]) + exp(lpsiMI[3]) + exp(lpsiMI[4]) + exp(lpsiMI[5]) + exp(lpsiMI[6]))) * season[t]
    psiMI[2,t] <- (exp(lpsiMI[2]) / (1 + exp(lpsiMI[1]) + exp(lpsiMI[2]) + exp(lpsiMI[3]) + exp(lpsiMI[4]) + exp(lpsiMI[5]) + exp(lpsiMI[6]))) * season[t]
    psiMI[3,t] <- (exp(lpsiMI[3]) / (1 + exp(lpsiMI[1]) + exp(lpsiMI[2]) + exp(lpsiMI[3]) + exp(lpsiMI[4]) + exp(lpsiMI[5]) + exp(lpsiMI[6]))) * season[t]
    psiMI[4,t] <- (exp(lpsiMI[4]) / (1 + exp(lpsiMI[1]) + exp(lpsiMI[2]) + exp(lpsiMI[3]) + exp(lpsiMI[4]) + exp(lpsiMI[5]) + exp(lpsiMI[6]))) * season[t]
    psiMI[5,t] <- (exp(lpsiMI[5]) / (1 + exp(lpsiMI[1]) + exp(lpsiMI[2]) + exp(lpsiMI[3]) + exp(lpsiMI[4]) + exp(lpsiMI[5]) + exp(lpsiMI[6]))) * season[t]
    psiMI[6,t] <- (exp(lpsiMI[6]) / (1 + exp(lpsiMI[1]) + exp(lpsiMI[2]) + exp(lpsiMI[3]) + exp(lpsiMI[4]) + exp(lpsiMI[5]) + exp(lpsiMI[6]))) * season[t]
    psiMI[7,t] <- 1 - psiMI[1,t] - psiMI[2,t] - psiMI[3,t] - psiMI[4,t] - psiMI[5,t] - psiMI[6,t] 
  }
  ...
  for (i in 4:6){
    lpsiCC[i] ~ dnorm(0, 0.001)
  }
  
  for (t in 1:(n.occasions-1)){
    psiCC[1,t] <- (exp(lpsiCC[1]) / (1 + exp(lpsiCC[1]) + exp(lpsiCC[2]) + exp(lpsiCC[3]) + exp(lpsiCC[4]) + exp(lpsiCC[5]) + exp(lpsiCC[6]))) * season[t]
    psiCC[2,t] <- (exp(lpsiCC[2]) / (1 + exp(lpsiCC[1]) + exp(lpsiCC[2]) + exp(lpsiCC[3]) + exp(lpsiCC[4]) + exp(lpsiCC[5]) + exp(lpsiCC[6]))) * season[t]
    psiCC[3,t] <- (exp(lpsiCC[3]) / (1 + exp(lpsiCC[1]) + exp(lpsiCC[2]) + exp(lpsiCC[3]) + exp(lpsiCC[4]) + exp(lpsiCC[5]) + exp(lpsiCC[6]))) * season[t]
    psiCC[4,t] <- (exp(lpsiCC[4]) / (1 + exp(lpsiCC[1]) + exp(lpsiCC[2]) + exp(lpsiCC[3]) + exp(lpsiCC[4]) + exp(lpsiCC[5]) + exp(lpsiCC[6]))) * season[t]
    psiCC[5,t] <- (exp(lpsiCC[5]) / (1 + exp(lpsiCC[1]) + exp(lpsiCC[2]) + exp(lpsiCC[3]) + exp(lpsiCC[4]) + exp(lpsiCC[5]) + exp(lpsiCC[6]))) * season[t]
    psiCC[6,t] <- (exp(lpsiCC[6]) / (1 + exp(lpsiCC[1]) + exp(lpsiCC[2]) + exp(lpsiCC[3]) + exp(lpsiCC[4]) + exp(lpsiCC[5]) + exp(lpsiCC[6]))) * season[t]
    psiCC[7,t] <- 1 - psiCC[1,t] - psiCC[2,t] - psiCC[3,t] - psiCC[4,t] - psiCC[5,t] - psiCC[6,t] 
  }
  
  for (i in 1:6){
    lpsiSE[i] ~ dnorm(0, 0.001)
  }

  for (t in 1:(n.occasions-1)){
    psiSE[1,t] <- (exp(lpsiSE[1]) / (1 + exp(lpsiSE[1]) + exp(lpsiSE[2]) + exp(lpsiSE[3]) + exp(lpsiSE[4]) + exp(lpsiSE[5]) + exp(lpsiSE[6]))) * season[t]
    psiSE[2,t] <- (exp(lpsiSE[2]) / (1 + exp(lpsiSE[1]) + exp(lpsiSE[2]) + exp(lpsiSE[3]) + exp(lpsiSE[4]) + exp(lpsiSE[5]) + exp(lpsiSE[6]))) * season[t]
    psiSE[3,t] <- (exp(lpsiSE[3]) / (1 + exp(lpsiSE[1]) + exp(lpsiSE[2]) + exp(lpsiSE[3]) + exp(lpsiSE[4]) + exp(lpsiSE[5]) + exp(lpsiSE[6]))) * season[t]
    psiSE[4,t] <- (exp(lpsiSE[4]) / (1 + exp(lpsiSE[1]) + exp(lpsiSE[2]) + exp(lpsiSE[3]) + exp(lpsiSE[4]) + exp(lpsiSE[5]) + exp(lpsiSE[6]))) * season[t]
    psiSE[5,t] <- (exp(lpsiSE[5]) / (1 + exp(lpsiSE[1]) + exp(lpsiSE[2]) + exp(lpsiSE[3]) + exp(lpsiSE[4]) + exp(lpsiSE[5]) + exp(lpsiSE[6]))) * season[t]
    psiSE[6,t] <- (exp(lpsiSE[6]) / (1 + exp(lpsiSE[1]) + exp(lpsiSE[2]) + exp(lpsiSE[3]) + exp(lpsiSE[4]) + exp(lpsiSE[5]) + exp(lpsiSE[6]))) * season[t]
    psiSE[7,t] <- 1 - psiSE[1,t] - psiSE[2,t] - psiSE[3,t] - psiSE[4,t] - psiSE[5,t] - psiSE[6,t] 
  }
  
  for (i in 1:6){
    lpsiBR[i] ~ dnorm(0, 0.001)
  }

  for (t in 1:(n.occasions-1)){
    psiBR[1,t] <- (exp(lpsiBR[1]) / (1 + exp(lpsiBR[1]) + exp(lpsiBR[2]) + exp(lpsiBR[3]) + exp(lpsiBR[4]) + exp(lpsiBR[5]) + exp(lpsiBR[6]))) * season[t]
    psiBR[2,t] <- (exp(lpsiBR[2]) / (1 + exp(lpsiBR[1]) + exp(lpsiBR[2]) + exp(lpsiBR[3]) + exp(lpsiBR[4]) + exp(lpsiBR[5]) + exp(lpsiBR[6]))) * season[t]
    psiBR[3,t] <- (exp(lpsiBR[3]) / (1 + exp(lpsiBR[1]) + exp(lpsiBR[2]) + exp(lpsiBR[3]) + exp(lpsiBR[4]) + exp(lpsiBR[5]) + exp(lpsiBR[6]))) * season[t]
    psiBR[4,t] <- (exp(lpsiBR[4]) / (1 + exp(lpsiBR[1]) + exp(lpsiBR[2]) + exp(lpsiBR[3]) + exp(lpsiBR[4]) + exp(lpsiBR[5]) + exp(lpsiBR[6]))) * season[t]
    psiBR[5,t] <- (exp(lpsiBR[5]) / (1 + exp(lpsiBR[1]) + exp(lpsiBR[2]) + exp(lpsiBR[3]) + exp(lpsiBR[4]) + exp(lpsiBR[5]) + exp(lpsiBR[6]))) * season[t]
    psiBR[6,t] <- (exp(lpsiBR[6]) / (1 + exp(lpsiBR[1]) + exp(lpsiBR[2]) + exp(lpsiBR[3]) + exp(lpsiBR[4]) + exp(lpsiBR[5]) + exp(lpsiBR[6]))) * season[t]
    psiBR[7,t] <- 1 - psiBR[1,t] - psiBR[2,t] - psiBR[3,t] - psiBR[4,t] - psiBR[5,t] - psiBR[6,t] 
  }
  ...
  for (t in 1:(n.occasions-1)){
    psiAR[1,t] <- (exp(lpsiAR[1]) / (1 + exp(lpsiAR[1]) + exp(lpsiAR[2]) + exp(lpsiAR[3]) + exp(lpsiAR[4]) + exp(lpsiAR[5]) + exp(lpsiAR[6]))) * season[t]
    psiAR[2,t] <- (exp(lpsiAR[2]) / (1 + exp(lpsiAR[1]) + exp(lpsiAR[2]) + exp(lpsiAR[3]) + exp(lpsiAR[4]) + exp(lpsiAR[5]) + exp(lpsiAR[6]))) * season[t]
    psiAR[3,t] <- (exp(lpsiAR[3]) / (1 + exp(lpsiAR[1]) + exp(lpsiAR[2]) + exp(lpsiAR[3]) + exp(lpsiAR[4]) + exp(lpsiAR[5]) + exp(lpsiAR[6]))) * season[t]
    psiAR[4,t] <- (exp(lpsiAR[4]) / (1 + exp(lpsiAR[1]) + exp(lpsiAR[2]) + exp(lpsiAR[3]) + exp(lpsiAR[4]) + exp(lpsiAR[5]) + exp(lpsiAR[6]))) * season[t]
    psiAR[5,t] <- (exp(lpsiAR[5]) / (1 + exp(lpsiAR[1]) + exp(lpsiAR[2]) + exp(lpsiAR[3]) + exp(lpsiAR[4]) + exp(lpsiAR[5]) + exp(lpsiAR[6]))) * season[t]
    psiAR[6,t] <- (exp(lpsiAR[6]) / (1 + exp(lpsiAR[1]) + exp(lpsiAR[2]) + exp(lpsiAR[3]) + exp(lpsiAR[4]) + exp(lpsiAR[5]) + exp(lpsiAR[6]))) * season[t]
    psiAR[7,t] <- 1 - psiAR[1,t] - psiAR[2,t] - psiAR[3,t] - psiAR[4,t] - psiAR[5,t] - psiAR[6,t] 
  }
...
inits <- function(){list(mean.phiDB = runif(4, 0, 1), mean.phiJB = runif(4, 0, 1), mean.phiMI = runif(4, 0, 1),
                         ...
                         lpsiDB = c(NA, rnorm(5)), lpsiJB = c(NA, NA, NA, rnorm(3)),
                         lpsiMI = c(NA, NA, NA, rnorm(3)), lpsiCC = c(NA, NA, NA, rnorm(3)),
                         lpsiSE = rnorm(6), lpsiBR = rnorm(6), lpsiAR = rnorm(6))}  
...
saveRDS(sim.marr$summary, file = paste0("./analysis-output/debugging/multistate-debugging5-summary", Sys.Date(), ".rds"))

saveRDS(sim.marr$sims.list, file = paste0("./analysis-output/debugging/multistate-debugging5-simslist", Sys.Date(), ".rds"))
