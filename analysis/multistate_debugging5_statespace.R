...
model {
  ...
  for (t in 1:(n.occasions-1)){
    psiDB[1,t] <- (db[1]/sum(db[])) * season[t]
    ...
    psiDB[7,t] <- (db[7]/sum(db[])) * season[t]
  }
  ...
  for (t in 1:(n.occasions-1)){
    psiJB[1,t] <- (jb[1]/sum(jb[])) * season[t]
    psiJB[2,t] <- (jb[2]/sum(jb[])) * season[t]
    psiJB[3,t] <- (jb[3]/sum(jb[])) * season[t]
    psiJB[4,t] <- (jb[4]/sum(jb[])) * season[t]
    psiJB[5,t] <- (jb[5]/sum(jb[])) * season[t]
    psiJB[6,t] <- (jb[6]/sum(jb[])) * season[t]
    psiJB[7,t] <- (jb[7]/sum(jb[])) * season[t]
  }
  ...
  for (t in 1:(n.occasions-1)){
    psiMI[1,t] <- (mi[1]/sum(mi[])) * season[t]
    psiMI[2,t] <- (mi[2]/sum(mi[])) * season[t]
    psiMI[3,t] <- (mi[3]/sum(mi[])) * season[t]
    psiMI[4,t] <- (mi[4]/sum(mi[])) * season[t]
    psiMI[5,t] <- (mi[5]/sum(mi[])) * season[t]
    psiMI[6,t] <- (mi[6]/sum(mi[])) * season[t]
    psiMI[7,t] <- (mi[7]/sum(mi[])) * season[t]
  }
...
for (t in 1:(n.occasions-1)){
    psiCC[1,t] <- (cc[1]/sum(cc[])) * season[t]
    psiCC[2,t] <- (cc[2]/sum(cc[])) * season[t]
    psiCC[3,t] <- (cc[3]/sum(cc[])) * season[t]
  ...
  for (i in 1:nind){
  for (t in 1:(n.occasions-1)){
    ps[1,i,t,1] <- phiDB[t] * psiDB[1,t]
    ps[1,i,t,2] <- phiDB[t] * psiDB[2,t]
    ps[1,i,t,3] <- phiDB[t] * psiDB[3,t]
    ps[1,i,t,4] <- phiDB[t] * psiDB[4,t]
    ps[1,i,t,5] <- phiDB[t] * psiDB[5,t]
    ps[1,i,t,6] <- phiDB[t] * psiDB[6,t]
    ps[1,i,t,7] <- phiDB[t] * psiDB[7,t]
    ps[1,i,t,8] <- 1-phiDB[t]
    
    ps[2,i,t,1] <- phiJB[t] * psiJB[1,t]
    ps[2,i,t,2] <- phiJB[t] * psiJB[2,t]
    ps[2,i,t,3] <- phiJB[t] * psiJB[3,t]
    ps[2,i,t,4] <- phiJB[t] * psiJB[4,t]
    ps[2,i,t,5] <- phiJB[t] * psiJB[5,t]
    ps[2,i,t,6] <- phiJB[t] * psiJB[6,t]
    ps[2,i,t,7] <- phiJB[t] * psiJB[7,t]
    ps[2,i,t,8] <- 1-phiJB[t]
    
    ps[3,i,t,1] <- phiMI[t] * psiMI[1,t]
    ps[3,i,t,2] <- phiMI[t] * psiMI[2,t]
    ps[3,i,t,3] <- phiMI[t] * psiMI[3,t]
    ps[3,i,t,4] <- phiMI[t] * psiMI[4,t]
    ps[3,i,t,5] <- phiMI[t] * psiMI[5,t]
    ps[3,i,t,6] <- phiMI[t] * psiMI[6,t]
    ps[3,i,t,7] <- phiMI[t] * psiMI[7,t]
    ps[3,i,t,8] <- 1-phiMI[t]
    
    ps[4,i,t,1] <- phiCC[t] * psiCC[1,t]
    ps[4,i,t,2] <- phiCC[t] * psiCC[2,t]
    ps[4,i,t,3] <- phiCC[t] * psiCC[3,t]
    ps[4,i,t,4] <- phiCC[t] * psiCC[4,t]
    ps[4,i,t,5] <- phiCC[t] * psiCC[5,t]
    ps[4,i,t,6] <- phiCC[t] * psiCC[6,t]
    ps[4,i,t,7] <- phiCC[t] * psiCC[7,t]
    ps[4,i,t,8] <- 1-phiCC[t]
    
    ps[5,i,t,1] <- phiSE[t] * psiSE[1,t]
    ps[5,i,t,2] <- phiSE[t] * psiSE[2,t]
    ps[5,i,t,3] <- phiSE[t] * psiSE[3,t]
    ps[5,i,t,4] <- phiSE[t] * psiSE[4,t]
    ps[5,i,t,5] <- phiSE[t] * psiSE[5,t]
    ps[5,i,t,6] <- phiSE[t] * psiSE[6,t]
    ps[5,i,t,7] <- phiSE[t] * psiSE[7,t]
    ps[5,i,t,8] <- 1-phiSE[t]
    
    ps[6,i,t,1] <- phiBR[t] * psiBR[1,t]
    ps[6,i,t,2] <- phiBR[t] * psiBR[2,t]
    ps[6,i,t,3] <- phiBR[t] * psiBR[3,t]
    ps[6,i,t,4] <- phiBR[t] * psiBR[4,t]
    ps[6,i,t,5] <- phiBR[t] * psiBR[5,t]
    ps[6,i,t,6] <- phiBR[t] * psiBR[6,t]
    ps[6,i,t,7] <- phiBR[t] * psiBR[7,t]
    ps[6,i,t,8] <- 1-phiBR[t]
    
    ps[7,i,t,1] <- phiAR[t] * psiAR[1,t]
    ps[7,i,t,2] <- phiAR[t] * psiAR[2,t]
    ps[7,i,t,3] <- phiAR[t] * psiAR[3,t]
    ps[7,i,t,4] <- phiAR[t] * psiAR[4,t]
    ps[7,i,t,5] <- phiAR[t] * psiAR[5,t]
    ps[7,i,t,6] <- phiAR[t] * psiAR[6,t]
    ps[7,i,t,7] <- phiAR[t] * psiAR[7,t]
    ps[7,i,t,8] <- 1-phiAR[t]
...
jags.data <- list(y = CH, f = f, n.occasions = dim(CH)[2], nind = dim(CH)[1],
                  z = known.state.ms(CH, 8), season = rep(c(1, 2, 3, 4), length.out = n.occasions-1))
...
inits <- function(){list(mean.phiDB = runif(4, 0, 1), mean.phiJB = runif(4, 0, 1), mean.phiMI = runif(4, 0, 1),
                         ...
                         db = c(NA, rgamma(5, 1, 1), NA), jb = c(NA, NA, NA, rgamma(3, 1, 1), NA),
                         mi = c(NA, NA, NA, rgamma(3, 1, 1), NA), cc = c(NA, NA, NA, rgamma(4, 1, 1)),
                         se = rgamma(7, 1, 1), br = rgamma(7, 1, 1), ar = rgamma(7, 1, 1),
                         z = ms.init.z(CH, f))}  
