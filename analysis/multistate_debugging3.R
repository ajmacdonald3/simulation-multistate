...
psiDB.DB <- 0
psiDB.JB <- rep(c(0.25, 0, 0.25, 0.25), length.out = n.occasions)
psiDB.MI <- rep(c(0.15, 0.4, 0.15, 0.15), length.out = n.occasions)
psiDB.CC <- 0.1
psiDB.SE <- 0.3
psiDB.BR <- 0.2
psiDB.AR <- 0
...
for (i in 1:totrel){
  for (t in 1:(n.occasions-1)){
    PSI.STATE[,,i,t] <- matrix(c(
      phiDB[t]*psiDB.DB, phiDB[t]*psiDB.JB[t], phiDB[t]*psiDB.MI[t], phiDB[t]*psiDB.CC, phiDB[t]*psiDB.SE, phiDB[t]*psiDB.BR, phiDB[t]*psiDB.AR, 1-phiDB[t],
...
model {
  ...
  for (t in 1:(n.occasions-1)){
    ...
    psiDB[2,t] <- (db[2]/sum(db[])) * transDB[t]
    psiDB[3,t] <- (db[3]/sum(db[])) * transDB[t]
    psiDB[4,t] <- (db[4]/sum(db[]))
    psiDB[5,t] <- (db[5]/sum(db[]))
    psiDB[6,t] <- (db[6]/sum(db[]))
...
jags.data <- list(marr = marr, n.occasions = ncol(CH), rel = rowSums(marr),
                  ...
                  transDB = rep(c(1, 2, 3, 4), length.out = n.occasions-1))
...
saveRDS(sim.marr$summary, file = paste0("./analysis-output/debugging/multistate-debugging3-summary", Sys.Date(), ".rds"))

saveRDS(sim.marr$sims.list, file = paste0("./analysis-output/debugging/multistate-debugging3-simslist", Sys.Date(), ".rds"))

# parameter identifiability checks
summary <- readRDS("./analysis-output/debugging/multistate-debugging3-summary2021-11-01.rds")
sims.list <- readRDS("./analysis-output/debugging/multistate-debugging3-simslist2021-11-01.rds")
...
for (i in colnames(sims.list)){
  ...
  png(filename = paste0("analysis-output/parameter-identifiability/debugging3/",
                        i,"-","check.png"),
...
png(filename = "figures/debugging/debugging3/multistate-debugging3.ppcheck.png", width = 8, height = 6,
    units = "in", res = 600)
...
# transitions
psiDB.sim <- tibble(site = c("DB-DB", "DB-CC", "DB-SE", "DB-BR", "DB-AR"),
                    value = c(psiDB.DB, psiDB.CC, psiDB.SE, psiDB.BR, psiDB.AR))

psiDBt.sim <- tibble(site = c("DB-JB1", "DB-JB2", "DB-JB3", "DB-JB4", "DB-MI1", "DB-MI2", "DB-MI3", "DB-MI4"),
                     value = c(0.25, 0, 0.25, 0.25, 0.15, 0.4, 0.15, 0.15))
...
psiAR.sim <- tibble(site = c("AR-DB", "AR-JB", "AR-MI", "AR-CC", "AR-SE", "AR-BR", "AR-AR"),
...
psiDB.mod <- sims.list %>% 
  as.data.frame() %>% 
  select(psiDB.1, psiDB.4, psiDB.5, psiDB.6, psiDB.7) %>% 
...
psiDBt.mod <- sims.list %>% 
  as.data.frame() %>% 
  select(psiDB.2, psiDB.3, psiDB.9, psiDB.10, psiDB.17, psiDB.18, psiDB.24, psiDB.25) %>% 
  pivot_longer(cols = 1:8, names_to = "site", values_to = "estimate") %>%
  mutate(site = str_replace(site, "psiDB.2$", "DB-JB1")) %>%
  mutate(site = str_replace(site, "psiDB.3$", "DB-MI1")) %>%
  mutate(site = str_replace(site, "psiDB.9$", "DB-JB2")) %>%
  mutate(site = str_replace(site, "psiDB.10$", "DB-MI2")) %>%
  mutate(site = str_replace(site, "psiDB.17$", "DB-JB3")) %>% 
  mutate(site = str_replace(site, "psiDB.18$", "DB-MI3")) %>%
  mutate(site = str_replace(site, "psiDB.24$", "DB-JB4")) %>%
  mutate(site = str_replace(site, "psiDB.25$", "DB-MI4"))
...
psiAR.mod <- sims.list %>% 
  ...
  pivot_longer(cols = 1:7, names_to = "site", values_to = "estimate") %>%
  mutate(site = str_replace(site, "psiAR.1", "AR-DB")) %>%
...
psiDBt.plot <- ggplot() +
  geom_violin(psiDBt.mod,
              mapping = aes(x = site, y = estimate, group = site,
                            fill = site), alpha = 0.6) +
  geom_boxplot(psiDBt.mod, mapping = aes(x = site, y = estimate, group = site),
               width = .05, fill = NA, outlier.colour = NA) +
  stat_summary(psiDBt.mod, mapping = aes(x = site, y = estimate, group = site),
               fun = median, geom = "point", fill = "white", shape = 21, size = 2.5) +
  geom_point(psiDBt.sim,
             mapping = aes(x = site, y = value), size = 2, colour = "black") +
  scale_fill_viridis(discrete = TRUE) +
  xlab("Transition") +
  ylab("Transition probability") +
...
# plot all
# survival
png(filename = "figures/debugging/debugging3/phiDB.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/phiJB.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/phiMI.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/phiCC.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/phiSE.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/phiBR.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/phiAR.png", width = 8, height = 8,
    units = "in", res = 600)
...
# resighting
png(filename = "figures/debugging/debugging3/pDB.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/pJB.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/pMI.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/pCC.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/pSE.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/pBR.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/pAR.png", width = 8, height = 8,
    units = "in", res = 600)
...
# transition
png(filename = "figures/debugging/debugging3/psiDB.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/psiDBt.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/psiJB.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/psiMI.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/psiCC.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/psiSE.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/psiBR.png", width = 8, height = 8,
    units = "in", res = 600)
...
png(filename = "figures/debugging/debugging3/psiAR.png", width = 8, height = 8,
    units = "in", res = 600)
