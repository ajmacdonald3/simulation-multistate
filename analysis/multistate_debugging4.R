...
model {
  ...
  for (t in 1:(n.occasions-1)){
    ...
    psiDB[4,t] <- (db[4]/sum(db[])) * transDB[t]
    psiDB[5,t] <- (db[5]/sum(db[])) * transDB[t]
    psiDB[6,t] <- (db[6]/sum(db[])) * transDB[t]
  ...
  for (t in 1:(n.occasions-1)){
    ...
    psiJB[4,t] <- (jb[4]/sum(jb[])) * transJB[t]
    psiJB[5,t] <- (jb[5]/sum(jb[])) * transJB[t]
    psiJB[6,t] <- (jb[6]/sum(jb[])) * transJB[t]
  ...
  for (t in 1:(n.occasions-1)){
    ...
    psiMI[4,t] <- (mi[4]/sum(mi[])) * transMI[t]
    psiMI[5,t] <- (mi[5]/sum(mi[])) * transMI[t]
    psiMI[6,t] <- (mi[6]/sum(mi[])) * transMI[t]
...
for (t in 1:(n.occasions-1)){
    ...
    psiCC[4,t] <- (cc[4]/sum(cc[])) * transCCCC[t]
    psiCC[5,t] <- (cc[5]/sum(cc[])) * transCCSE[t]
    psiCC[6,t] <- (cc[6]/sum(cc[])) * transCCBR[t]
    psiCC[7,t] <- (cc[7]/sum(cc[])) * transCCAR[t]
  }
  ...
  for (t in 1:(n.occasions-1)){
    ...
    psiBR[2,t] <- (br[2]/sum(br[])) * transBRJB[t]
    psiBR[3,t] <- (br[3]/sum(br[])) * transBRMI[t]
    psiBR[4,t] <- (br[4]/sum(br[])) * transBRCC[t]
    psiBR[5,t] <- (br[5]/sum(br[])) * transBRSE[t]
    psiBR[6,t] <- (br[6]/sum(br[])) * transBRBR[t]
    psiBR[7,t] <- (br[7]/sum(br[])) * transBRAR[t]
  }
  ...
  for (t in 1:(n.occasions-1)){
    psiAR[1,t] <- (ar[1]/sum(ar[])) * transARDB[t]
    psiAR[2,t] <- (ar[2]/sum(ar[])) * transARJB[t]
    psiAR[3,t] <- (ar[3]/sum(ar[])) * transARMI[t]
    psiAR[4,t] <- (ar[4]/sum(ar[])) * transARCC[t]
    psiAR[5,t] <- (ar[5]/sum(ar[])) * transARSE[t]
    psiAR[6,t] <- (ar[6]/sum(ar[])) * transARBR[t]
    psiAR[7,t] <- (ar[7]/sum(ar[])) * transARAR[t]
  }
...
jags.data <- list(marr = marr, n.occasions = ncol(CH), rel = rowSums(marr),
                  ...
                  transCCSE = rep(c(0, 1, 2, 0), length.out = n.occasions-1),
                  transCCBR = rep(c(0, 1, 2, 0), length.out = n.occasions-1),
                  ...
                  transSECC = rep(c(1, 2, 0, 0), length.out = n.occasions-1),
                  transSESE = rep(c(1, 2, 3, 4), length.out = n.occasions-1),
                  transSEBR = rep(c(1, 2, 3, 4), length.out = n.occasions-1),
                  transSEAR = rep(c(0, 0, 1, 2), length.out = n.occasions-1),
                  transBRDB = rep(c(0, 0, 0, 1), length.out = n.occasions-1),
                  transBRJB = rep(c(1, 0, 0, 0), length.out = n.occasions-1),
                  transBRMI = rep(c(1, 0, 0, 0), length.out = n.occasions-1),
                  transBRCC = rep(c(1, 2, 0, 0), length.out = n.occasions-1),
                  transBRSE = rep(c(1, 2, 3, 4), length.out = n.occasions-1),
                  transBRBR = rep(c(1, 2, 3, 4), length.out = n.occasions-1),
                  transBRAR = rep(c(0, 0, 1, 2), length.out = n.occasions-1),
                  transARDB = rep(c(0, 0, 0, 1), length.out = n.occasions-1),
                  transARJB = rep(c(1, 0, 0, 0), length.out = n.occasions-1),
                  transARMI = rep(c(1, 0, 0, 0), length.out = n.occasions-1),
                  transARCC = rep(c(1, 0, 0, 0), length.out = n.occasions-1),
                  transARSE = rep(c(1, 0, 0, 2), length.out = n.occasions-1),
                  transARBR = rep(c(1, 0, 0, 2), length.out = n.occasions-1),
                  transARAR = rep(c(0, 0, 0, 1), length.out = n.occasions-1))
