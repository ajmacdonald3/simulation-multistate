...
for (t in 1:(n.occasions-2)){
   ...
   for (j in (t+1):(n.occasions-1)){
      Q[(t-1)*ns+(1:ns), (j-1)*ns+(1:ns)] <- Q[(t-1)*ns+(1:ns), (j-2)*ns+(1:ns)] %*% (ps[,t,] * qo[,t,])
      }
   }
...
# Define the cell probabilities of the multistate m-array  
    # The main diagonal
for (t in 1:(n.occasions-2)){
   pr[(t-1)*ns+(1:ns),(t-1)*ns+(1:ns)] <- Q[(t-1)*ns+(1:ns),(t-1)*ns+(1:ns)] %*% (ps[,t,] * po[,t,])
   # Above main diagonal
   for (j in (t+1):(n.occasions-1)){
      pr[(t-1)*ns+(1:ns), (j-1)*ns+(1:ns)] <- Q[(t-1)*ns+(1:ns), (j-1)*ns+(1:ns)] %*% (ps[,j,] * po[,j,])
      }
   }
pr[(n.occasions-2)*ns+(1:ns), (n.occasions-2)*ns+(1:ns)] <- ps[,n.occasions-1,] * po[,n.occasions-1,]
...
# Initial values 
inits <- function(){list(phiA = runif(1, 0, 1), phiB = runif(1, 0, 1), phiC = runif(1, 0, 1),
                         lpsiA = rnorm(2), lpsiB = rnorm(2), lpsiC = rnorm(2),
                         pA = runif(1, 0, 1) , pB = runif(1, 0, 1) , pC = runif(1, 0, 1))}  
